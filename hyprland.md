...
| 通知管理   | sudo pacman -S dunst             | dunstify 需要安装 libnotify |
...
| 简化版 man | sudo pacman -S tldr                                 | tldr --update          |
...
1.字体模糊, 在 hyprland.conf 中添加以下内容
...
```bash
编辑 /usr/share/applications/code.desktop
在 Exec 中添加 --enable-wayland-ime
```

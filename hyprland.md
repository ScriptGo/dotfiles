...
| 系统信息 | sudo pacman -S btop fastfetch                       |      |
...
如果上面的命令生成的是英文目录，就不用输入下面的命令了；
如果生成的是中文目录，则需要输入下面的命令！

```bash
export LANG=en_US.UTF-8
xdg-user-dirs-gtk-update # 回车后点击 Update 按钮
...
2.编辑 /usr/share/sddm/themes/corners/ 目录下的 theme.conf 文件:
